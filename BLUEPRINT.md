...
6. **`hem.kepimpinan` kosong — tetapi datanya sudah ada dalam HADIR** — dikenal pasti 6 September 2026 oleh pemilik. Ia tidak memerlukan tab `dash_kepimpinan` manual seperti yang diandaikan dahulu.
    ...
    **Penghalang sebenar ialah kemasukan data — dan semasa menyemaknya, sesuatu yang lebih besar ditemui.** Disemak terus pada spreadsheet `Sistem Kehadiran Sepadan` 6 September 2026.

    Tab `main` ialah tab **SENARAI KESELURUHAN MURID** bersusun atur idME (62 lajur), dan ia memang mempunyai lajur `JAWATAN MURID`. Ini disahkan daripada kod, bukan tekaan: `hadirSenaraiMurid_` membaca `main` secara **kedudukan lajur** — `r[2]` nama, `r[3]` IC, `r[9]` tahun, `r[10]` kelas (`HadirWeb.gs:429-445`) — yang hanya sepadan dengan susun atur idME.

    **`main` mengandungi 33 murid sahaja: 28 Tahun Satu dan 5 Tahun Dua, semuanya kelas BIJAK.** Lajur BIL berjalan 1 hingga 33 dan berakhir bersih sebelum tab berikutnya, jadi ini bukan eksport terpotong. Daripada 33 itu, **1 mempunyai `KETUA KELAS`** dan 32 kosong.
