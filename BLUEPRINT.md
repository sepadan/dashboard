...
Mulai v1.5.0, admin mempunyai **Tetapan Murid** mengikut kelas. Status RMT
ditulis ke tab `rmt`; jawatan Pengawas/Pengawas Perpustakaan/Ketua Kelas/
Penolong Ketua Kelas/Murid Biasa ditulis pada lajur tambahan bertajuk
`JAWATAN MURID` dalam `main`. Data Murid dibuka melalui kad nama baca sahaja;
butang Edit mengaktifkan medan. Kelas dipaparkan sebagai `1 Bijak`, manakala
tahun dan jantina dilengkapkan daripada sumber data sedia ada.

Mulai v1.9.0, admin mempunyai **Tetapan Guru** untuk menambah, nyahaktif,
upload CSV, mencari senarai dan menyelaras semula. HADIR menyimpan status dalam
`HADIR_GURU`, kemudian memanggil API rasmi AKSI dan SEMAK. `merge` menambah atau
mengemas kini; `sync` menyamakan senarai aktif. Rekod tidak aktif kekal supaya
kata laluan, tugasan dan sejarah sistem sasaran tidak terpadam. Kawalan kemas
kini guru di kedua-dua sistem sasaran kekal berfungsi.

Cache Storage PWA mencache cangkerang statik sahaja dan tidak menyimpan jawapan
API. Mulai v1.6.2, `localStorage` menyimpan satu salinan data paparan guru bagi
hari semasa supaya sembilan kelas muncul serta-merta, kemudian rangkaian
menyegarkannya di latar. Salinan luput apabila tarikh berubah, dipaksa ke mod
guru, tidak mengandungi IC/PIN/token/hak admin dan bersifat baca sahaja sehingga
data terkini tiba. Paparan guru menerima nama serta kunci harian legap; IC/MyKid
...
Apps Script Version 61 menyediakan `apiImportGuru` dengan mod `merge`/`sync`.
Nama baharu ditambah secara pukal; nama yang tiada dalam sync penuh ditanda
tidak aktif tanpa membuang kata laluan, tugasan, markah atau sejarah. Tetapan
guru SEMAK sendiri kekal dan perubahan tempatan turut direlay melalui HADIR.
...
AKSI kini mempunyai PWA v1.3.1 yang lengkap dan sudah diterbitkan: ikon
...
alamat API. Versi pada sidebar ialah `AKSI v1.3.1 · PWA`; ujian tanpa pelayan
...
Mulai 27 Ogos 2026, Apps Script Version 8 meluaskan perlindungan transaksi
kepada semua laluan mutasi yang diaudit: Pencapaian, Keahlian, Kelab, Laporan,
Murid, Arkib, Tetapan, Setup dan Auth, di samping Kehadiran/Penilaian/PAJSK.
Import murid dan keahlian mengesah semua baris sebelum mengubah helaian; IC
tidak sah/berulang atau keahlian kategori berulang membatalkan import secara
atomik. ID laporan, pencapaian dan gambar menggunakan maksimum + 1, tulisan
berulang dipukalkan, dan cache anggaran dibuang selepas data rujukan berubah.
Apps Script Version 8 membalas identiti produksi `ok:true` sebagai
`AKSI v1.3.1` dalam 2.514 saat. Pencetus sandaran `backupAutoTrigger` kini aktif
setiap Ahad 2–3 pagi bersama `cuciSesiLama` harian. GitHub commit `06092e8` dan
Pages run #35 berjaya; produksi memaparkan v1.3.1 dengan cache
`20260826-9`, tanpa URL Apps Script dalam cache.

Apps Script Version 11 menambah status dan kontrak `importGuru` merge/sync.
Ia menerima nama/jawatan, mengemas kini secara pukal, menjana ID guru stabil,
menyahaktif tanpa padam fizikal dan mengekalkan akaun/kata laluan/tugasan serta
sejarah sedia ada. Padam admin kini bermaksud nyahaktif dan direlay melalui HADIR.
...
| Repo | Kandungan | Nota |
|---|---|---|
| `sepadan/semak` | PWA di akar + `index.html` (iframe) + `src/` (5 fail) | SEMAK v1.2.0; Apps Script Version 61, status/merge/sync guru tanpa menyentuh markah/kata laluan |
| `sepadan/aksi` | kod Apps Script di akar + `docs/` | Lihat 2.3 |
| `sepadan/hadir` | PWA di akar + backend pemasangan dalam `apps-script/` | HADIR v1.9.0; Semak Kehadiran muka depan; penyelaras pusat murid/guru; migrasi guru mengutamakan SEMAK; berkongsi backend KEHADIRAN |
...
Upload dalam mana-mana sistem menghantar data asas melalui relay HADIR dan API
rasmi sasaran. Data dari AKSI/SEMAK masuk ke `main` sebagai `merge` sahaja;
pengarkiban/pembuangan hanya autoritatif dari HADIR kerana sistem sasaran boleh
sengaja tidak membawa PRA/PPKI. Setiap sync memulangkan status setiap aplikasi
dan merekod bilangan sahaja tanpa nama/IC dalam `HADIR_LOG`.

### 3.10b Senarai aktif guru boleh ditetapkan dari mana-mana sistem

HADIR ialah penyelaras pusat, tetapi HADIR, AKSI atau SEMAK boleh memulakan
perubahan. Tambah/edit menggunakan `merge`; nyahaktif atau sync penuh menghantar
snapshot senarai aktif. Nama yang tiada dalam snapshot ditanda `TIDAK AKTIF`
pada semua sistem—baris tidak dipadam secara fizikal. Kata laluan, tugasan,
markah, kokurikulum dan sejarah kekal tempatan. Nama yang sama boleh diaktifkan
semula kemudian. CSV sync penuh wajib menunjukkan pratonton dan meminta
pengesahan. HADIR memegang satu `ScriptLock` sepanjang perubahan pusat dan
penghantaran, maka operasi yang selesai paling akhir menjadi keadaan terkini.

### 3.10c Data sama, peraturan sistem kekal

Relay hanya menyelaraskan medan asas identiti murid dan senarai guru. Ia tidak
menyalin kehadiran, markah, peperiksaan, tugasan, keahlian, pencapaian, PAJSK,
akaun atau kata laluan. Setiap penerima mesti menggunakan API rasminya sendiri,
jadi pengecualian kelas AKSI, calon/cache SEMAK dan peraturan KEHADIRAN terus
dipatuhi. Rahsia bersama `SEPADAN_SYNC_SECRET` hanya berada dalam Script
Properties ketiga-tiga projek. Penanda asal `HADIR`/`AKSI`/`SEMAK` wajib supaya
penerima tidak menghantar semula muatan yang sama dan mencipta gelung.
...
6. **`hem.kepimpinan` kosong — tetapi datanya sudah ada dalam HADIR** — dikenal pasti 6 September 2026 oleh pemilik. Ia tidak memerlukan tab `dash_kepimpinan` manual seperti yang diandaikan dahulu.

    **Sumber:** lajur `JAWATAN MURID` dalam tab `main` KEHADIRAN, ditulis oleh **Tetapan Murid** HADIR sejak v1.5.0 (`hadirSimpanTetapanMurid_`, `apps-script/HadirWeb.gs:522`). Lima nilai dibenarkan: `PENGAWAS`, `PENGAWAS PERPUSTAKAAN`, `KETUA KELAS`, `PENOLONG KETUA KELAS`, `MURID BIASA`.

    **Sasaran:** `hem.kepimpinan` menerima `[{jawatan, murid}]` — kiraan agregat sahaja. Mengira baris `main` mengikut lajur itu (kecuali `MURID BIASA`) memenuhi kontrak tanpa menghantar satu pun nama, jadi peraturan 2 tidak dilanggar.

    **Preseden:** `hem.bantuan` RMT sudah pun datang dari tab `rmt` yang diurus HADIR, jadi penjana memang sudah membaca data kendalian HADIR. Ini perubahan berbentuk sama, dalam `Dashboard.gs` repo privat `sistem-kehadiran-sepadan` — bukan dalam mana-mana repo awam.

    **Skop ditetapkan pemilik 6 September 2026: empat jawatan sahaja** — Pengawas, Pengawas Perpustakaan, Ketua Kelas, Penolong Ketua Kelas. SLB, PRS dan ketua rumah sukan **tidak** termasuk. Senarai izin HADIR sudah tepat sama dengan keempat-empat ini, jadi HADIR tidak perlu diubah langsung. `PANDUAN-DATA.md` yang menyenaraikan jawatan lebih luas telah dibetulkan pada tarikh sama.

    **Yang penjana perlu buat:** kira baris tab `main` mengikut `JAWATAN MURID`, abaikan `MURID BIASA` dan sel kosong, keluarkan empat baris `{jawatan, murid}` walaupun kiraannya sifar supaya carta kekal stabil bentuknya.

    **Penghalang sebenar ialah kemasukan data, bukan kod — dan ia belum bermula.** Disemak terus pada spreadsheet `Sistem Kehadiran Sepadan` 6 September 2026: tab `main` memegang **223 murid merentas 9 kelas** dengan lima lajur sahaja — `BIL`, `NAMA MURID`, `KELAS`, `STATUS RMT`, `IC`. Lajur `JAWATAN MURID` **tidak wujud di situ**. Jadi liputannya 0 daripada 223, bukan sekadar tidak lengkap.

    Tiada apa yang rosak: `hadirSimpanTetapanMurid_` mencipta lajur itu pada simpanan pertama melalui Tetapan Murid (`HadirWeb.gs:551-555`). Ia belum wujud kerana belum ada sesiapa menyimpan jawatan.

    **Satu rekod terputus.** Tab berasingan bersusun atur idME penuh (62 lajur, 33 murid kelas BIJAK sahaja) mempunyai lajur `JAWATAN MURID` dengan **satu** entri `KETUA KELAS`. HADIR tidak membaca tab itu, jadi rekod itu takkan berpindah sendiri dan perlu ditaip semula dalam `main`.

    **Cara termurah mengisi:** tambah lajur bernama tepat `JAWATAN MURID` di hujung `main` dan taip nilainya terus dalam Google Sheets — tiada kod, tiada CSV, boleh dibuat dari telefon. Ejaan mesti tepat (`PENGAWAS`, `PENGAWAS PERPUSTAKAAN`, `KETUA KELAS`, `PENOLONG KETUA KELAS`); sel kosong dibaca sebagai `MURID BIASA`. Membaiki penjana dahulu akan menghasilkan carta yang memaparkan angka separa tanpa apa-apa tanda ia separa — pembaca akan menganggap 3 pengawas bermakna sekolah ada 3 pengawas. Carta kosong jujur; carta separa menipu. Jadi **jangan sambungkan penjana sebelum kemasukan data selesai**, atau sertakan penunjuk liputan (contohnya satu baris `Belum ditetapkan`) supaya pembaca nampak berapa banyak yang belum dikira.

    **Kemasukan pukal mungkin ada, belum disahkan.** `hadirUploadMuridCsv_` menghantar setiap lajur CSV melalui medan `semua`, jadi fail CSV yang mempunyai lajur `JAWATAN MURID` berkemungkinan mengisinya sekali gus. Fungsi yang benar-benar menulis ke sheet — `simpanSenaraiMuridUpload` — berada dalam projek KEHADIRAN privat, jadi ini tidak dapat disahkan dari repo awam. Ujilah dengan CSV dua baris dalam mod **merge** dahulu; mod `sync` menggantikan senarai penuh
7. ~~Kata laluan lalai SEMAK dan AKSI belum ditukar~~ — **bukan isu; keputusan pemilik, disahkan 6 September 2026.** Kata laluan sengaja dikekalkan ringkas dan dikongsi supaya guru boleh mengingatinya. Kata laluan yang tidak boleh diingat berakhir tertampal atas meja, dan itu lebih teruk. Jangan buka semula isu ini atau cadangkan dasar kata laluan tanpa arahan pemilik.

    Nilai sebenar dibuang daripada dokumen ini pada 6 September 2026. Ia bukan perubahan dasar — kata laluan kekal sama dan kekal mudah — cuma fail ini awam di `sepadan.github.io/dashboard/BLUEPRINT.md` dan menyiarkan nilainya bersebelahan alamat kedua-dua sistem tidak memberi apa-apa faedah kepada pembaca yang sah. Guru yang perlu tahu sudah tahu.
...
21. **Import CSV idME HADIR ditolak** — `Fungsi tidak dibenarkan.` Muka depan HADIR v1.3.0 mempunyai butang *Update Data Murid CSV idME*, tetapi fungsi backendnya sama ada tiada dalam senarai izin projek KEHADIRAN, atau deployment Apps Script belum dibuat semula selepas kod ditampal. **Tiada data terjejas** — penolakan berlaku sebelum sebarang penulisan. Diagnosis: bandingkan nama fungsi yang dipanggil `app.js` dengan senarai izin dalam `HadirWeb.gs`, kemudian sahkan versi deployment
    ...
    **Semakan 6 September 2026:** butang itu kini memanggil `uploadMuridCsv`, dan nama itu ada dalam senarai izin `apps-script/HadirWeb.gs` (baris 29). Punca asal — fungsi tiada dalam senarai izin — nampaknya sudah hilang sejak relay v1.8.x. Isu dikekalkan terbuka sehingga seorang admin benar-benar memuat naik satu fail CSV idME dan berjaya
...
25. ~~Kestabilan menyeluruh AKSI dan sandaran automatik~~ — **selesai dan diterbitkan 27 Ogos 2026.** AKSI v1.3.1 mengunci semua laluan mutasi yang diaudit, menjadikan import murid/keahlian atomik, menstabilkan ID Laporan/Pencapaian/gambar, memukalkan tulisan Auth/Laporan dan mengosongkan cache anggaran selepas data rujukan berubah. Apps Script Version 8 diterbitkan pada URL sama; endpoint identiti `ok:true`; semua fail GS dan dua suite regresi lulus. `backupAutoTrigger` aktif setiap Ahad 2–3 pagi, sementara `cuciSesiLama` kekal harian dengan kadar ralat terakhir 0%. GitHub `06092e8`, Pages run #35 dan cache produksi `20260826-9` disahkan. Pengesahan operasi tulis data sebenar kekal di isu #14
26. ~~Tetapan Guru pusat dalam HADIR~~ — **selesai dan diterbitkan 28 Ogos 2026.** HADIR v1.7.0 menambah senarai/carian, tambah seorang, upload CSV dan selaras semula. Import gabung-sahaja mengekalkan guru, jawatan kosong, kata laluan serta fungsi kemas kini tempatan. Backend HADIR Version 106, AKSI Version 9 dan SEMAK Version 59 diterbitkan pada URL sedia ada. Ujian sintaks/parser/merge/kunci lulus; endpoint produksi disahkan dengan token/kata laluan palsu supaya tiada data guru sebenar ditulis
27. ~~Upload data murid/guru dari mana-mana sistem~~ — **selesai dan diterbitkan 28 Ogos 2026.** HADIR v1.8.0 menjadi relay berahsia; AKSI v1.4.0 dan SEMAK v1.1.0 menghantar upload tempatan ke HADIR, yang menggabung sumber induk lalu menyelaras aplikasi ketiga. Penanda asal menghalang gelung. Hanya data asas diselaraskan; semua peraturan, markah, kehadiran, kokurikulum, tugasan dan kata laluan kekal tempatan. Apps Script HADIR 107, AKSI 10 dan SEMAK 60 menggunakan URL sedia ada
28. ~~Senarai guru lama AKSI/SEMAK tidak muncul dalam HADIR~~ — **selesai 28 Ogos 2026.** Relay v1.8.0 hanya bertindak pada simpanan/upload baharu, jadi data lama tidak mempunyai peristiwa untuk dihantar. HADIR v1.8.1 menjadikan butang Selaras membina kesatuan guru sedia ada AKSI dan SEMAK sekali apabila `HADIR_GURU` kosong, menggabungkannya di HADIR dan menyebarkan melalui API rasmi. Selepas migrasi awal, sync dua hala biasa mengambil alih; kata laluan dan tetapan setiap sistem kekal tempatan
29. ~~Sumber migrasi guru awal belum ditetapkan~~ — **selesai 28 Ogos 2026.** Pengguna mengesahkan senarai SEMAK ialah yang paling baharu. HADIR v1.8.2 mengambil SEMAK dahulu sebagai benih `HADIR_GURU`; AKSI hanya sandaran jika SEMAK gagal atau kosong. Sync berikutnya kekal merge-only supaya kata laluan/tugasan/rekod tempatan tidak terpadam
30. ~~Senarai guru mesti mengikut sistem yang dikemas kini terakhir~~ — **selesai dan diterbitkan 29 Ogos 2026.** HADIR v1.9.0, AKSI v1.5.0 dan SEMAK v1.2.0 menambah status aktif/tidak aktif, mod merge/sync, nyahaktif tanpa padam fizikal serta snapshot aktif melalui relay pusat. CSV sync penuh mempunyai pratonton/pengesahan. Kata laluan, tugasan, markah, kokurikulum dan sejarah kekal di sistem asal. Apps Script HADIR 110, AKSI 11 dan SEMAK 61 diterbitkan pada URL sedia ada; ujian teknikal tidak mengubah senarai guru produksi
31. ~~Pembaikan audit kestabilan 30 Ogos belum diterbitkan ke produksi~~ — **selesai 30 Ogos 2026, disahkan 6 September 2026.** Ketiga-tiga jejari merekod penerbitan pada URL sedia ada: AKSI Apps Script Version 12, HADIR Version 111 dan SEMAK Version 62. Isu ini kekal terbuka di hab selama seminggu selepas kerjanya siap — sebabnya jejari dikemas kini tetapi hab tidak, iaitu tepat hanyutan yang bahagian 9 cuba halang
32. ~~Kehadiran hari ini kosong walaupun sudah ditanda~~ — **punca dikenal pasti 24 Ogos 2026.** Tajuk tab sebenar ialah `24/08`, tetapi penjana membandingkannya dengan teks `24/8`. `kehadiranHariIni_()` dibaiki supaya menerima `D/M`, `DD/MM` dan tarikh yang turut mempunyai tahun. *(Entri ini dahulunya turut bernombor 21. Nombor 21 dalam log perubahan versi 4.1 merujuk kepada import CSV idME, jadi entri ini dinomborkan semula pada 6 September 2026.)*

33. **AKSI memuat semula halaman pada pemasangan PWA pertama** — `docs/js/pwa.js` memuat semula pada setiap `controllerchange`, tetapi `clients.claim()` dalam Service Worker membangkitkannya pada lawatan pertama juga, bukan hanya apabila versi baharu menggantikan yang lama. Kesannya: setiap panggilan API halaman berjalan dua kali (disukat 14 permintaan bagi satu lawatan, sepatutnya 7) dan mana-mana POST yang sedang berjalan dibatalkan. Simpan logo ialah mangsa paling kerap kerana muatannya besar; Safari iOS melaporkan pembatalan itu sebagai `Load failed`, yang kelihatan seperti ralat pelayan sedangkan pelayan tidak pernah menerima permintaan. Turut dijumpai: sepuluh halaman AKSI meminta `manifest.webmanifest?v=1.4.0` dan ikon `?v=1.3.1` sedangkan APP_SHELL hanya menyimpan `?v=1.5.0`, dan `semak/index.html` meminta `pwa.js?v=20260828-5` sedangkan APP_SHELL menyimpan `-6` — setiap URL tidak sepadan terlepas cache dan gagal ketika luar talian. **Dibaiki dalam branch `claude/check-all-app-repos-wsipny`** dengan ujian versi aset baharu bagi AKSI dan SEMAK (HADIR sudah ada yang setara — sebab itu HADIR tidak hanyut). Menunggu gabung ke `main` dan penerbitan GitHub Pages. Tiada perubahan Apps Script diperlukan
...
34. ~~Kehadiran September `data.json` kekal 0~~ — **bukan pepijat.** Disahkan oleh pemilik 6 September 2026: **1–4 September ialah cuti sekolah**, jadi tiada kehadiran untuk direkod dan `{"bulan": "Sep", "murid": 0}` memang betul. Angka itu akan terisi sendiri apabila persekolahan bermula semula.
    ...
    Dicatat supaya sesi akan datang tidak mengejarnya lagi: tolakan `data.json` pada 2, 4 dan 5 September hanya membawa perubahan angka akademik (kemasukan markah SEMAK), bukan kehadiran — itu konsisten dengan cuti, bukan dengan penjana yang rosak. Sebelum menganggap bulan bernilai sifar sebagai pepijat, semak kalendar persekolahan dahulu.
...
|---|---|---|
| 5.8 | 6 September 2026 | Isu #31 ditutup: ketiga-tiga jejari merekod penerbitan 30 Ogos (AKSI Version 12, HADIR 111, SEMAK 62) sedangkan hab masih menyenaraikannya sebagai belum diterbitkan. Isu #33 dibuka dan dibaiki dalam branch: AKSI memuat semula halaman pada pemasangan PWA pertama, menggandakan panggilan API dan membatalkan POST yang sedang berjalan — puncanya `Load failed` ketika simpan logo. Versi aset AKSI/SEMAK diselaraskan dengan APP_SHELL dan ujian versi aset ditambah bagi kedua-duanya. Isu #34 dibuka dan terus ditutup: kehadiran September 0 disahkan pemilik sebagai cuti sekolah 1–4 September, bukan pepijat. Isu #7 ditutup sebagai keputusan pemilik: kata laluan ringkas dikongsi memang disengajakan supaya guru dapat mengingatinya; nilai sebenar dibuang daripada dokumen awam ini tanpa mengubah kata laluan itu sendiri. Isu #6 ditulis semula: `hem.kepimpinan` tidak memerlukan tab manual — jawatan murid sudah disimpan HADIR dalam lajur `JAWATAN MURID` tab `main`, dan `hem.bantuan` RMT membuktikan penjana sudah membaca data kendalian HADIR. Pemilik menetapkan skop kepimpinan kepada empat jawatan (Pengawas, Pengawas Perpustakaan, Ketua Kelas, Penolong Ketua Kelas), yang sudah tepat sama dengan senarai izin HADIR; `PANDUAN-DATA.md` dibetulkan kerana ia menyenaraikan jawatan lebih luas. Semakan terus pada spreadsheet mengesahkan liputan jawatan ialah 0 daripada 223 murid — lajur `JAWATAN MURID` belum wujud dalam `main`, dan satu entri sedia ada berada dalam tab lain yang tidak dibaca HADIR. Daftar isu dinombor semula — dua entri bernombor 21 dan entri 20 tersalah letak |
| 5.7 | 30 Ogos 2026 | Catat isu #31: pembaikan audit repo untuk integriti SEMAK, status guru/XSS AKSI dan had login HADIR telah diuji tempatan tetapi belum diterbitkan ke Apps Script/GitHub Pages produksi. Audit Dashboard kini menguatkuasakan allowlist skema penuh lima domain bagi setiap sesi, jenis nilai agregat, larangan medan luar kontrak/senarai individu, serta IC/MyKad 12 digit termasuk format bersengkang atau berjarak |
| 5.6 | 29 Ogos 2026 | Isu #30 ditutup: senarai aktif guru kini boleh ditetapkan dari HADIR, AKSI atau SEMAK. Tambah/edit ialah merge; nyahaktif/sync penuh menghantar snapshot aktif melalui kunci pusat HADIR. Rekod tiada ditanda tidak aktif tanpa memadam kata laluan, tugasan atau sejarah. Versi paparan/cache HADIR v1.9.0, AKSI v1.5.0, SEMAK v1.2.0; Apps Script 110/11/61 |
| 5.5 | 28 Ogos 2026 | Isu #29 ditutup: pengguna mengesahkan SEMAK memegang senarai guru paling baharu. HADIR v1.8.2 menjadikan SEMAK sumber migrasi awal dan AKSI sandaran sahaja; data domain serta kata laluan kekal tempatan |
| 5.4 | 28 Ogos 2026 | Isu #28 ditutup: HADIR v1.8.1 menambah migrasi awal kesatuan guru sedia ada daripada AKSI dan SEMAK apabila `HADIR_GURU` kosong, kemudian menggunakan aliran merge-only biasa. Perubahan ini memindahkan nama/jawatan sahaja dan tidak menyentuh kata laluan atau tetapan sistem |
| 5.3 | 28 Ogos 2026 | Isu #27 ditutup: upload murid/guru boleh dibuat pada HADIR, AKSI atau SEMAK dan data asas diselaraskan melalui relay HADIR berahsia. API rasmi sasaran dan penanda asal mengekalkan syarat sistem serta mencegah gelung; pengarkiban murid hanya autoritatif dari HADIR. Versi paparan/cache: HADIR v1.8.0, AKSI v1.4.0, SEMAK v1.1.0; Apps Script 107/10/60 |
| 5.2 | 28 Ogos 2026 | Isu #26 ditutup: HADIR v1.7.0 menambah Tetapan Guru dan kontrak penyelarasan merge-only. AKSI mengekalkan rekod/jawatan kosong/akaun lama; SEMAK mengekalkan guru/kata laluan lama; kedua-duanya masih boleh dikemas kini terus. Apps Script HADIR 106, AKSI 9 dan SEMAK 59 diterbitkan. Pembaca RPC HADIR turut menerima padding Base64 Google `\x3d`. Ujian teknikal menggunakan kelayakan palsu sahaja dan tidak mengubah data produksi |
| 5.1 | 27 Ogos 2026 | Isu #25 ditutup: AKSI v1.3.1 meluaskan kunci kepada semua laluan mutasi, menjadikan import atomik, menstabilkan ID dan tulisan kelompok, membuang cache selepas perubahan serta mengaktifkan sandaran Ahad 2–3 pagi. Apps Script Version 8, endpoint produksi `ok:true`, GitHub `06092e8`, Pages run #35 dan cache `20260826-9` disahkan; semua fail GS dan dua suite regresi lulus. Isu #14 kekal untuk ujian tulis data sebenar |
...
| 4.5 | 25 Ogos 2026 | HADIR v1.5.0 menambah Tetapan Murid admin untuk RMT/jawatan, kad Data Murid baca sahaja sebelum Edit, kelas `1 Bijak`, pelengkapan tahun/jantina, Log Keluar di kaki menu dan nisbah RMT hadir/jumlah. Status RMT kekal dalam tab `rmt`; jawatan menggunakan lajur tambahan `JAWATAN MURID` dalam `main`. GitHub `9e05fc4`; Apps Script versi 101 diterbitkan pada URL sama. Produksi memuat 9 kelas dan Semak Kehadiran 24 Ogos memaparkan RMT `26/33` tanpa ralat konsol |
