...
6. **`hem.kepimpinan` kosong — tetapi datanya sudah ada dalam HADIR** — dikenal pasti 6 September 2026 oleh pemilik. Ia tidak memerlukan tab `dash_kepimpinan` manual seperti yang diandaikan dahulu.
    ...
    **Penghalang ialah kemasukan data.** Tab `main` ialah tab *SENARAI KESELURUHAN MURID* bersusun atur idME (62 lajur) dan ia **memang** memegang kesemua **223 murid** serta mempunyai lajur `JAWATAN MURID` (lajur terakhir, BI). Disahkan pemilik 6 September 2026 melalui tangkapan skrin: BIL berjalan 1 hingga 223.

    Pemilik mengesahkan jawatan **belum diisi sepenuhnya**. Bilangan tepat yang sudah diisi belum diketahui — lihat amaran di bawah tentang kenapa ia tidak dapat dikira dari luar.

    ⚠️ **Eksport Google Drive memotong baris bagi setiap tab.** Semasa menyemak isu ini, satu bacaan eksport memulangkan hanya **33 baris pertama** `main` lalu terus ke tab berikutnya — berakhir bersih pada BIL 33 sehingga kelihatan seperti tab penuh. Hadnya nampak sekitar 2,000 sel per tab: `main` (62 lajur) putus pada 33 baris, sedangkan `rmt` (5 lajur) lepas penuh 223 baris dan kelihatan normal. Kesimpulan awal bahawa `main` kehilangan lapan daripada sembilan kelas adalah **salah** dan datang daripada perangkap ini. Ini peraturan 3.12 dalam bentuk baharu: eksport Drive menyertai `raw.githubusercontent.com` dan folder peti masuk sebagai sumber yang memulangkan salinan separa dengan yakin. Untuk mengira apa-apa dalam tab besar, kira di dalam Sheets sendiri — contohnya `=COUNTIF(BI7:BI, "PENGAWAS")` — jangan kira dari eksport.

    **Tab `dash_kepimpinan` memang wujud**, dengan tajuk `JAWATAN | MURID` dan sifar baris data. Jadi penerangan asal isu ini betul: ia direka sebagai input manual HEM. Ada dua jalan:

      1. **Isi `dash_kepimpinan` terus** — empat baris jawatan dan bilangannya. Carta terus hidup, tiada kod langsung. Kosnya kemas kini manual setiap kali bilangan berubah.
      2. **Jana daripada lajur `JAWATAN MURID` dalam `main`** — perubahan `Dashboard.gs` dalam repo privat, dan hanya berbaloi selepas jawatan benar-benar diisi. Sebelum itu carta akan memaparkan angka separa yang kelihatan muktamad.
...
|---|---|---|
| 5.8 | 6 September 2026 | Isu #31 ditutup: ketiga-tiga jejari merekod penerbitan 30 Ogos (AKSI Version 12, HADIR 111, SEMAK 62) sedangkan hab masih menyenaraikannya sebagai belum diterbitkan. Isu #33 dibuka dan dibaiki dalam branch: AKSI memuat semula halaman pada pemasangan PWA pertama, menggandakan panggilan API dan membatalkan POST yang sedang berjalan — puncanya `Load failed` ketika simpan logo. Versi aset AKSI/SEMAK diselaraskan dengan APP_SHELL dan ujian versi aset ditambah bagi kedua-duanya. Isu #34 dibuka dan terus ditutup: kehadiran September 0 disahkan pemilik sebagai cuti sekolah 1–4 September, bukan pepijat. Isu #7 ditutup sebagai keputusan pemilik: kata laluan ringkas dikongsi memang disengajakan supaya guru dapat mengingatinya; nilai sebenar dibuang daripada dokumen awam ini tanpa mengubah kata laluan itu sendiri. Isu #6 ditulis semula: `hem.kepimpinan` tidak memerlukan tab manual — jawatan murid sudah disimpan HADIR dalam lajur `JAWATAN MURID` tab `main`, dan `hem.bantuan` RMT membuktikan penjana sudah membaca data kendalian HADIR. Pemilik menetapkan skop kepimpinan kepada empat jawatan (Pengawas, Pengawas Perpustakaan, Ketua Kelas, Penolong Ketua Kelas), yang sudah tepat sama dengan senarai izin HADIR; `PANDUAN-DATA.md` dibetulkan kerana ia menyenaraikan jawatan lebih luas. Tab `main` disahkan memegang kesemua 223 murid dengan lajur `JAWATAN MURID`, dan tab `dash_kepimpinan` wujud tetapi kosong. Ditambah amaran alat: eksport Google Drive memotong baris setiap tab sekitar 2,000 sel dan berakhir kelihatan lengkap — satu bacaan begitu menghasilkan kesimpulan salah bahawa `main` kehilangan lapan kelas. Daftar isu dinombor semula — dua entri bernombor 21 dan entri 20 tersalah letak |
