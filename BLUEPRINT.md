...
34. **Kehadiran September `data.json` kekal 0** — `kehadiran.bulanan` memaparkan `{"bulan": "Sep", "murid": 0}` walaupun `data.json` ditolak pada 2, 4 dan 5 September 2026. Diff ketiga-tiga tolakan itu menunjukkan hanya angka akademik berubah; angka kehadiran September tidak pernah naik dari sifar. (`hari_ini: null` pada 6 September memang betul — hari Ahad.) Ia berlaku tepat pada pertukaran bulan, kelas pepijat yang sama seperti isu #32. Kod pengagregat berada dalam repo privat `sistem-kehadiran-sepadan`, jadi ia tidak dapat disemak dari repo awam. **Tindakan:** sahkan sama ada kehadiran 1–4 September memang sudah ditanda dalam KEHADIRAN. Jika ya, semak semula perbandingan tajuk tab bulan dalam penjana
